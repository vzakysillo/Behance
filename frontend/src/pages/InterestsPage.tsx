...
export default function InterestsPage() {
  ...
  useEffect(() => {
    ...
    getMe()
      .then((user) => {
        if (user.skills?.length) { navigate(routes.profile.root(), { replace: true }); return; }
        setChecking(false);
      })
      .catch(() => {
  ...
  const persistAndLeave = async (skills: string[]) => {
    ...
    try {
      if (skills.length > 0) await updateMe({ skills });
      navigate(routes.home());
    } catch (err) {
      setMessage(typeof err === "string" ? err : err instanceof Error ? err.message : "Could not save your interests. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (checking) {
    return (
      <main className="min-h-svh w-[min(100vw,1920px)] mx-auto relative overflow-hidden bg-white text-[#525252] font-['Inter',system-ui,sans-serif] text-center">
        <p className="absolute inset-0 grid place-items-center text-[#575656] text-xl font-medium">Loading...</p>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-svh w-[min(100vw,1920px)] mx-auto relative overflow-hidden bg-white text-[#525252] font-['Inter',system-ui,sans-serif] text-center
      ...
      >
        <h1
          id="interests-title"
          className="m-0 text-[#525252] text-[50px] font-bold leading-[1.2]
                     max-[1160px]:text-[clamp(34px,6vw,50px)] max-[640px]:text-[34px]"
        >
        ...
        <p className="mt-[70px] text-[#575656] text-xl font-medium leading-[1.2]
                      max-[1160px]:mt-[42px] max-[640px]:mt-7 max-[640px]:text-[17px]">
            ...
            return (
              <button
                ...
                className={[
                  ...
                  isSelected
                    ? "bg-[#575656] border-[#575656]"
                    : "bg-[#bdbdbd] border-transparent",
                ].join(" ")}
              >
                {interest}
              </button>
        ...
        {/* Footer */}
        <div
          className="absolute right-[50px] bottom-[115px] grid justify-items-end gap-[14px]
