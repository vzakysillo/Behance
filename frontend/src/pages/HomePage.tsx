...
export default function HomePage() {
  ...
  const filteredProjects = projects.filter((project) => {
    ...
    const matchesCategory = selectedCategory === "All" || project.name.toLowerCase().includes(selectedCategory.toLowerCase());
    return matchesSearch && matchesCategory;
  });

  if (loading)
    return (
      <div className="flex justify-center items-center min-h-[400px] text-lg font-['Inter',sans-serif] text-[#666]">
        Loading...
      </div>
    );
  ...
  if (error)
    return (
      <div className="flex justify-center items-center min-h-[400px] text-lg font-['Inter',sans-serif] text-[#d32f2f]">
        {error}
      </div>
    );

  return (
    <div className="min-h-screen bg-white p-0 ml-[200px]">
      {/* Top Bar */}
            ...
            >
              ...
              <div className="pt-[15px]">
                <div className="flex items-center gap-[10px] mb-2">
                  ...
                  <span className="text-base font-['Inter',sans-serif] font-medium text-white">Username</span>
                </div>
                <h3 className="text-xl font-['Inter',sans-serif] font-semibold text-white m-0 mb-2">{project.name}</h3>
                <p className="text-xl font-['Inter',sans-serif] font-normal text-white m-0 mb-[15px] leading-[1.4]">
                  ...
                  {[
                    { icon: (
                      <svg width="18" height="16" viewBox="0 0 18 16" fill="none">
                        <path d="M9 1C4.58 1 1 4.58 1 9C1 13.42 4.58 17 9 17C13.42 17 17 13.42 17 9C17 4.58 13.42 1 9 1ZM9 15.5C5.41 15.5 2.5 12.59 2.5 9C2.5 5.41 5.41 2.5 9 2.5C12.59 2.5 15.5 5.41 15.5 9C15.5 12.59 12.59 15.5 9 15.5Z" fill="white" />
                        <path d="M6 8C6 8 7.5 5.5 9 5.5C10.5 5.5 12 8 12 8C12 8 10.5 10.5 9 10.5C7.5 10.5 6 8 6 8Z" fill="white" />
                      </svg>
                    ), label: "8k" },
                    { icon: (
                    ...
                    <div key={i} className="flex items-center gap-2">
                      {stat.icon}
                      <span className="text-xl font-['Inter',sans-serif] font-medium text-white">{stat.label}</span>
                    </div>
                  ))}
                </div>
              </div>
